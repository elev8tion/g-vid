/**
 * ESM version of audio-analyzer (with timing cues)
 */
...
- Exact window: 8 seconds starting at ${context.trimWindow?.start?.toFixed?.(1) || '0.0'}s of the original recording.
...
- Critical: The mouth shapes, jaw, tongue, and micro-expressions must match the phonemes and exact timing of this specific recording. Do not generate generic or improved singing — replicate the raw, human performance in the file with perfect accuracy.`;
      ...
      return fallbackAnalysis(request, context);
    }
  },
};

async function analyzeWithFfmpeg(audioPath) {
...
function fallbackAnalysis(request, context) {
...
- Window: 8 seconds from ${context.trimWindow?.start?.toFixed?.(1) || '0.0'}s
